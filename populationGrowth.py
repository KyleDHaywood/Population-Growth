...
def dearAcheiveMyScriptDoesNotContainADingDangInfiniteLoopTYVM():
    ...
    for j in range(2024, 2029):
        births = secondsPerYear // birthRate
        deaths = secondsPerYear // deathRate
        immigrants = secondsPerYear // immigrationRate

        population += births - deaths + immigrants
        newBirths += births
        totaldeaths += deaths
        newImmigrants += immigrants

        daysEllapsed += secondsPerYear // 86400
        totalGrowth = newBirths - deaths + newImmigrants
        print(f'By the end of year {j}, we will have seen {math.ceil(daysEllapsed)} days, The countries total population: {population}, New births: {newBirths}, Deaths: {totaldeaths}, New immigrants: {newImmigrants}, Total growth {totalGrowth}\n')

dearAcheiveMyScriptDoesNotContainADingDangInfiniteLoopTYVM()